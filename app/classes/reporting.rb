class Reporting
  def chart_data(server, time_period)
    #
    ...
    query << "from heartbeats where server_id = #{server.id} and created_at >= current_timestamp - interval '1 hour hour' "
    ...
    if (time_period == '2') # 24 hours
      ...
      query << "from heartbeats where server_id = #{server.id} and created_at >= current_timestamp - interval '1 day' "
      query << "group by date_trunc('hour', created_at) "
      query << "order by date_trunc('hour', created_at)"
    elsif (time_period == '3') # 1 week
      query = "select date_trunc('hour', created_at) as created_at, round(avg(cpu_usage)::numeric, 2) as cpu_usage, round(avg((mem_used::numeric(10,2)/mem_total::numeric(10,2))*100), 2) as mem_usage "
      query << "from heartbeats where server_id = #{server.id} and created_at >= current_timestamp - interval '1 week' "
      query << "group by date_trunc('hour', created_at) "
      query << "order by date_trunc('hour', created_at)"
    elsif (time_period == '4') # 1 month
      query = "select date_trunc('hour', created_at) as created_at, round(avg(cpu_usage)::numeric, 2) as cpu_usage, round(avg((mem_used::numeric(10,2)/mem_total::numeric(10,2))*100), 2) as mem_usage "
      query << "from heartbeats where server_id = #{server.id} and created_at >= current_timestamp - interval '1 month' "
      query << "group by date_trunc('hour', created_at) "
      query << "order by date_trunc('hour', created_at)"
    end
